...
-module(emqx_omp_test_api_helpers).
...
create_connector(Connector) ->
    case emqx_omp_test_helpers:api_post(connectors, Connector) of
        {ok, _} ->
            ok;
        {error, Error} ->
            error(Error)
...
list_connectors() ->
    case emqx_omp_test_helpers:api_get(connectors) of
...
upload_plugin(Filename) ->
    ...
    case
        emqx_omp_test_helpers:api_post_raw(
            {plugins, install},
            [],
            {multipart, Parts}
        )
...
start_plugin(PluginId) ->
    case emqx_omp_test_helpers:api_put_raw({plugins, PluginId, start}, <<>>) of
        {ok, _} ->
            ok;
        {error, Error} ->
            error(Error)
...
stop_plugin(PluginId) ->
    case emqx_omp_test_helpers:api_put_raw({plugins, PluginId, stop}, <<>>) of
        {ok, _} ->
            ok;
        {error, Error} ->
            error(Error)
...
delete_plugin(PluginId) ->
    emqx_omp_test_helpers:api_delete({plugins, PluginId}).
...
create_rule(Rule) ->
    case emqx_omp_test_helpers:api_post(rules, Rule) of
        {ok, _} ->
            ok;
        {error, Error} ->
            error(Error)
...
list_rules() ->
    case emqx_omp_test_helpers:api_get(<<"rules?limit=1000">>) of
...
delete_rule(RuleId) ->
    emqx_omp_test_helpers:api_delete({rules, RuleId}).
...
create_action(Action) ->
    case emqx_omp_test_helpers:api_post(actions, Action) of
        {ok, _} ->
            ok;
        {error, Error} ->
            error(Error)
...
list_actions() ->
    case emqx_omp_test_helpers:api_get(actions) of
...
delete_action(ActionId) ->
    emqx_omp_test_helpers:api_delete({actions, ActionId}).
